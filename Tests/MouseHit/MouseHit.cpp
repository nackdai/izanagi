	...
	izanagi::sample::CSampleCamera& camera)
{
	...
	static const izanagi::SVector triPoint[3] =
	{
...

	{
		...
		m_Triangles.vb->Lock(0, 0, (void**)&data, IZ_FALSE);
		{
			...
			{
				izanagi::SVector::Copy(data[i].pos, triPoint[i]);
	...
	// カメラ
	camera.Init(
		izanagi::CVector(0.0f, 5.0f, -30.0f, 1.0f),
...
void CMouseHit::RenderInternal(izanagi::CGraphicsDevice* device)
{
	...
	if (m_Shader->BeginPass(0)) {
		...
		const izanagi::SVector& clr = (m_IsCross ? color[1] : color[0]);
