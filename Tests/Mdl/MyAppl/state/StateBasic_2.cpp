...
IZ_BOOL CStateBasicEx2::Render(izanagi::CGraphicsDevice* device)
{
	Render3D();
	Render2D();
...
IZ_BOOL CStateBasicEx2::Destroy()
{
	return Leave();
}

IZ_BOOL CStateBasicEx2::Enter(izanagi::IMemoryAllocator* allocator, void* val)
{
	izanagi::CGraphicsDevice* pDevice = CMySystem::GetInstance().GetGraphicsDevice();
...
IZ_BOOL CStateBasicEx2::OnKeyDown(IZ_UINT nChar, IZ_UINT nRepCnt, IZ_UINT nFlags)
{
	if (nChar == 'P') {
		m_Timeline.Pause();
	}
...
IZ_BOOL CStateBasicEx2::OnMouseMove(IZ_UINT nFlags, IZ_INT x, IZ_INT y)
{
	return IZ_TRUE;
}

IZ_BOOL CStateBasicEx2::OnMouseWheel(IZ_UINT nFlags, IZ_SHORT zDelta, IZ_INT x, IZ_INT y)
{
	return IZ_TRUE;
}