...
IZ_BOOL CStateBasicBoy::Render(izanagi::graph::CGraphicsDevice* device)
{
	Render3D();
	Render2D();
...
IZ_BOOL CStateBasicBoy::Enter(izanagi::IMemoryAllocator* allocator, void* val)
{
	izanagi::graph::CGraphicsDevice* pDevice = CMySystem::GetInstance().GetGraphicsDevice();
	...
	{
		m_pTex = pDevice->CreateTextureFromFile("data/boy_10.dds", izanagi::graph::E_GRAPH_PIXEL_FMT_RGBA8);
