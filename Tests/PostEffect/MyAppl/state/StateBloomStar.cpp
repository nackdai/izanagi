...
IZ_BOOL CStateBloomStar::Init()
{
	return IZ_TRUE;
}

IZ_BOOL CStateBloomStar::Render(izanagi::CGraphicsDevice* device)
{
	Render3D();
	Render2D();
...
IZ_BOOL CStateBloomStar::Update()
{
	if (m_bBack) {
...
IZ_BOOL CStateBloomStar::Destroy()
{
	return IZ_TRUE;
}

IZ_BOOL CStateBloomStar::Enter(izanagi::IMemoryAllocator* allocator, void* val)
{
	m_bBack = IZ_FALSE;
...
IZ_BOOL CStateBloomStar::Leave()
{
	return IZ_TRUE;
}

IZ_BOOL CStateBloomStar::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == VK_BACK) {
		m_bBack = IZ_TRUE;
	}
