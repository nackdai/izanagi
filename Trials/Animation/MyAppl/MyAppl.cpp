    ...
    HWND hFocusWindow)
{
    ...
    // カメラ初期化
    {
        izanagi::math::CVector vecPos(0.0f, 0.5f, 2.0f, 1.0f);
        izanagi::math::CVector vecRef(0.0f, 0.5f, 0.0f, 1.0f);
    ...
    m_Character = CCharacter::CreateCharacter(
                    CMySystem::GetInstance().GetMemoryAllocator(),
                    CMySystem::GetInstance().GetGraphicsDevice());
    IZ_ASSERT(m_Character != IZ_NULL);
