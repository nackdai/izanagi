...
CMdlMeshSubset::CMdlMeshSubset()
{
	...
	m_RenderListItem.Init(this);

	m_IsVisisbile = IZ_TRUE;
...
namespace {
	...
	CIndexBuffer* _CreateIndexBuffer(
		...
		const S_MDL_MESH_SUBSET& sMeshSubset,
		IInputStream* pIn)
	{
		// Create IndexBuffer.
...
#if 1
IZ_BOOL CMdlMeshSubset::DebugRender(
	CGraphicsDevice* pDevice,
	const CMdlSkeleton* pSkeleton,
	IMdlRenderHandler* pRenderHandler)
{
