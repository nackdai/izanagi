...
namespace graph
{
    class CGraphicsDeviceAndroid : public CGraphicsDeviceGLES2 {
    public:
        CGraphicsDeviceAndroid();
        virtual ~CGraphicsDeviceAndroid();
    ...
    CGraphicsDevice* CGraphicsDevice::CreateGraphicsDevice(IMemoryAllocator* allocator)
    {
        ...
        CGraphicsDeviceAndroid* instance = IZ_NULL;

        // メモリ確保
        buf = (IZ_UINT8*)ALLOC_ZERO(allocator, sizeof(CGraphicsDeviceAndroid));
        VGOTO(buf != IZ_NULL, __EXIT__);

        // インスタンス作成
        instance = new(buf) CGraphicsDeviceAndroid;
        {
    ...
    // コンストラクタ
    CGraphicsDeviceAndroid::CGraphicsDeviceAndroid()
    {
    }
    ...
    // リセット
    IZ_BOOL CGraphicsDeviceAndroid::Reset(const void* initialParam)
    {
