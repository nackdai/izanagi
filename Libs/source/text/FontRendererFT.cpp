...
// 登録情報
struct CFontRendererFT::SRegInfo {
    const S_FNT_MAP* map;
...
* インスタンス作成
*/
CFontRenderer* CFontRendererFT::CreateFontRendererBmp(
    ...
    IInputStream* in)
{
    ...
    CFontRendererFT* pInstance = NULL;
    ...
    // 確保サイズ計算
    IZ_UINT nSize = sizeof(CFontRendererFT);
    {
    ...
    // インスタンス作成
    pInstance = new(pBuf) CFontRendererFT;
    {
        pBuf += sizeof(CFontRendererFT);
...
// 開放
void CFontRendererFT::InternalRelease()
{
    delete this;
...
// テクスチャ作成
IZ_BOOL CFontRendererFT::CreateTexture(IInputStream* in)
{
    IZ_BOOL ret = IZ_TRUE;
    ...
    // テクスチャサイズ
    IZ_UINT nWidth = nFontNumH * m_Header.fontHeight;
    IZ_UINT nHeight = nFontNumV * m_Header.fontHeight;
...
*/
IZ_BOOL CFontRendererFT::Register(const void* str)
{
    if (!BeginRegister()) {
        // 登録開始処理失敗
        return IZ_FALSE;
    ...
    IZ_COLOR color/*= IZ_COLOR_RGB(255, 255, 255, 255)*/)
{
    ...
    for (;;) {
        ...
        if (nCode == 0) {
            ...
            sDstPoint.x += m_Header.fontHeight;
            continue;
        }
        ...
        if (bIsLineFeed) {
            // 改行
            sDstPoint.x = nX;
            sDstPoint.y += m_Header.fontHeight;
...
* 登録されていない場合は、登録を行う
*/
IZ_BOOL CFontRendererFT::RenderImmediate(
    void* str,
    IZ_INT nX, IZ_INT nY,
    IZ_COLOR color/*= IZ_COLOR_RGB(255, 255, 255, 255)*/)
{
...
// フォント登録
CFontRendererFT::SRegInfo* CFontRendererFT::RegisterInternal(IZ_UINT code)
{
    // マップ情報取得
...
// フォント登録開始
IZ_BOOL CFontRendererFT::BeginRegister()
{
...
// フォント登録終了
void CFontRendererFT::EndRegister()
{
    m_SrcTex->Unlock(0);
...
// 登録情報取得
CFontRendererFT::SRegInfo* CFontRendererFT::GetRegInfo(IZ_UINT code)
{
...
// マップ情報取得
const S_FNT_MAP* CFontRendererFT::GetFontMap(IZ_UINT code)
{
...
// 転送元テクスチャにフォント描画
void CFontRendererFT::RenderToSrcTex(
    SRegInfo* pRegInfo,
    IZ_UINT code,
    IZ_BOOL needRegister)
{
    ...
    if (pRegInfo != IZ_NULL) {
...
#else
        IZ_UINT nDstFontWidth = m_Header.fontHeight;
#endif
        ...
        SIntRect sSrcRect;
        {
            ...
            sSrcRect.bottom = sSrcRect.top + m_Header.fontHeight;
        }
        ...
        SIntRect sDstRect;
        {
            if (pRegInfo->x < 0) {
                ...
                if (pos > m_SrcTex->GetWidth()) {
                    ...
                    m_ImageInfo.y += m_Header.fontHeight;
                }
            ...
            sDstRect.bottom = sDstRect.top + m_Header.fontHeight;
        }
        ...
        // コピー
        for (IZ_UINT y = 0; y < m_Header.fontHeight; ++y) {
...
// 転送
IZ_BOOL CFontRendererFT::UpdateSurface()
{
...
// フォント描画
void CFontRendererFT::RenderInternal(
    IZ_UINT code,
    CIntPoint* ptDst,
    IZ_COLOR color)
{
    ...
    if (pRegInfo == NULL) {
        ...
        SIntRect sDstRect;
        {
            ...
            if (m_Header.isFixedPitch) {
                sDstRect.left += (m_Header.fontHeight - pRegInfo->map->fontWidth) >> 1;
            }
#endif
...
#if 0
            ptDst->x += pRegInfo->map->fontWidth;
#else
            if (m_Header.isFixedPitch) {
                ptDst->x += m_Header.fontHeight;
            }
...
* ピクセル幅取得
*/
IZ_UINT CFontRendererFT::GetWidth(void* str)
{
    // NOTE
    // 複数行あるときは一番長い行について返す
...
* 最初の１文字のみ調べる
*/
IZ_BOOL CFontRendererFT::IsRegistered(IZ_UINT code)
{
    // 登録情報探す
    SRegInfo* pRegInfo = GetRegInfo(code);
