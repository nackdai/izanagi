...
namespace {
	...
	BOOL _SetValueFunctorArgsF(izanagi::S_SHD_PASS_ANN& sAnn, CGannotation ann)
	{
		...
		BOOL ret = ((p != NULL) && (num <= izanagi::SHD_FUNCTOR_ARGS_NUM));
		VRETURN(ret);
...
namespace {
	...
	BOOL _SetStateValue(izanagi::S_SHD_PASS_STATE& sState, CGstateassignment sa)
	{
		static struct {
			...
			{"ZWriteEnable",    _SetValueZWriteEnable},
			{"ZEnable",         _SetValueZEnable},
			{"ZFunc",           _SetValueZFunc},
		};
	...
	CGpass pass)
{
	...
	if (progVS != NULL) {
		...
		sPass.VSType = CShaderConvUtil::GetVtxShaderType(lpszEntryPoint);
	}
