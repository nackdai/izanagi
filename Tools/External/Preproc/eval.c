 ...
 *          or long long / unsigned long long (though imcomplete yet, for
 *          C99).
...
static int      evallex( void);
#if MODE != POST_STANDARD
static VAL_SIGN *   evalchar( char * const token);
static expr_t   evalone( char ** seq_pp, int wide, int * ucn8);
#endif
static VAL_SIGN *   evaleval( VAL_SIGN * valp, int op);
static expr_t   evalsval( VAL_SIGN ** valpp, expr_t v1, expr_t v2, int op);
#if HAVE_ULONG
static expr_t   evaluval( VAL_SIGN ** valpp, uexpr_t v1u, uexpr_t v2u, int op);
#endif
static void     overflow( const char * op_name, VAL_SIGN ** valpp);
#if OK_SIZE
static int      dosizeof( void);
static int      looktype( int typecode);
#endif
#if DEBUG_EVAL
static void     dumpval( const char * msg, const VAL_SIGN * valp);
static void     dumpstack( const OPTAB * opstack, const OPTAB * opp
        , const VAL_SIGN * value, const VAL_SIGN * valp);
#endif

#else   /* ! PROTO  */

static int      evallex();          /* Get type and value of token  */
#if MODE != POST_STANDARD
static VAL_SIGN *   evalchar();     /* Evaluate character constant  */
static expr_t   evalone();          /* Evaluate a character         */
#endif
static VAL_SIGN *   evaleval();     /* Entry to #if arithmetic      */
static expr_t   evalsval();         /* Do signed arithmetic of expr.*/
#if HAVE_ULONG
static expr_t   evaluval();         /* Do unsigned arithmetic       */
#endif
static void     overflow();         /* Diagnose overflow of expr.   */
#if OK_SIZE
static int      dosizeof();         /* Evaluate sizeof (type)       */
static int      looktype();         /* Look for type of the name    */
#endif
#if DEBUG_EVAL
static void     dumpval();          /* Print value of an operand    */
static void     dumpstack();        /* Print stacked operators      */
#endif
...
static D09 const char   opdope[ OP_END + 1] = {
...
static D09 VAL_SIGN     ev;     /* Current value and signedness     */
static D09 int          skip = 0;   /* 3-way signal of skipping expr*/
static const char * const   non_eval
        = " (in non-evaluated sub-expression)";             /* _W4_ */
...
static const TYPES basic_types[] = {
    ...
    { T_UNSIGNED,   "izsigned",     ANYFLOAT | ANYINT | ANYSIGN },
    { 0,            NULL,           0 }     /* Signal end           */
};
 ...
 * evaleval     Evaluates the current operator, given the values on the
...
#if DEBUG_EVAL
        if (debug & EXPRESSION)
            printf( "In eval loop skip = %d, binop = %d, line is: %s\n",
...
#if DEBUG_EVAL
        if (debug & EXPRESSION)
            printf( "op = %s, opdope = %04o, binop = %d, skip = %d\n",
        ...
        while (1) {
            ...
            if (opp->prec < prec) {
                if (op == OP_LPA) {
                    prec = OP_RPA_PREC;
#if MODE >= STANDARD
                    if ((warn_level & 2) && ++parens == exp_nestmin + 1)
                        cwarn(
                    "More than %.0s%ld nesting of parens"   /* _W2_ */
                            , NULLST, (long) exp_nestmin, NULLST);
#endif
                } else if (op == OP_QUE) {
                    prec = OP_QUE_PREC;
                } else if (is_unary( op)) {
...
#if DEBUG_EVAL
                if (debug & EXPRESSION) {
                    printf( "stacking %s, ", opname[ op]);
                    if (&value[0] < valp)
                        dumpval( "valp[-1].val == ", valp - 1);
                    printf( " at %s\n", infile->bptr);
 ...
 */
{
    ...
    case NAM:
        ...
        if (str_eq( identifier, "defined")) {   /* defined name     */
            ...
            if (scantoken( c, (workp = work, &workp), work_end) == NAM) {
                if (warn)
                    ev.val = (lookid( identifier) != NULL);
                if (c1 != '(' || skipws() == ')')   /* Balanced ?   */
                    return  VAL;            /* Parsed ok            */
            }
            cerror( "Bad defined syntax", NULLST, 0L, NULLST);      /* _E_  */
            break;
        }
#if MODE >= STANDARD
        if (cplus) {
         ...
         */
        if ((! skip && (warn_level & 2)) || (skip && (warn_level & 4)))
            cwarn( "Undefined symbol \"%s\"%.0ld%s" /* _W2_ _W4_    */
...
#endif
#if MODE == POST_STANDARD
        cerror( "Can't use the character constant %s"       /* _E_  */
                , work, 0L, NULLST);
        break;
#else
        valp = evalchar( work);             /* 'valp' points 'ev'   */
        if (valp->sign == VAL_ERROR)
            break;
#if DEBUG_EVAL
        if (debug & EXPRESSION) {
            dumpval( "evalchar returns ", &ev);
            fputc( '\n', stdout);
        }
#endif
    ...
    case NUM:                               /* Numbers are harder   */
        valp = evalnum( work);              /* 'valp' points 'ev'   */
        if (valp->sign == VAL_ERROR)
            break;
#if DEBUG_EVAL
        if (debug & EXPRESSION) {
            dumpval( "evalnum returns ", &ev);
            fputc( '\n', stdout);
        }
#endif
        return  VAL;
    ...
    default:                                /* Total nonsense       */
        cerror( "Can't use a character %.0s0x%02lx"         /* _E_  */
 ...
 */
{
    ...
    if (get_unexpandable( skipws(), warn) != OPE || openum != OP_LPA)
        goto  no_good;                      /* Not '('              */
    ...
    while (! type_end) {
        token_type = get_unexpandable( skipws(), warn);
                                /* Get next token expanding macros  */
        switch (token_type) {
        case OPE:
            if (openum == OP_LPA) {         /* thing (*)() func ptr */
                if (get_unexpandable( skipws(), warn) == OPE
                        && openum == OP_MUL
                        && get_unexpandable( skipws(), warn) == OPE
                        && openum == OP_RPA) {      /* (*)          */
                    if (get_unexpandable( skipws(), warn) != OPE
                            || openum != OP_LPA
                            || get_unexpandable( skipws(), warn) != OPE
        ...
        case NAM:                           /* Look for type comb.  */
            if ((typecode = looktype( typecode)) == 0)
...
#if MODE > CON_FALSE
    if (warn_level & 4)
        cwarn( "sizeof is disallowed in Standard"           /* _W4_ */
                , NULLST, 0L, NULLST);
#endif
    return  VAL;
...
#endif
{
    const char * const  unknown_type
            = "sizeof: Unknown type \"%s\"%.0ld%s";     /* _E_ _W4_ */
    const char * const  illeg_comb
        = "sizeof: Illegal type combination with \"%s\"%.0ld%s";    /* _E_ _W4_ */
    int     token_type;
    register const TYPES *  tp;

#if HAVE_LLONG || HAVE_LDBL
    if (str_eq( identifier, "long")) {
        if ((token_type
                = get_unexpandable( skipws(), !skip || (warn_level & 4)))
                == NO_TOKEN)
            return  typecode;
        if (token_type == NAM) {
#if HAVE_LLONG
        ...
        ungetstring( work, NULLST);         /* Not long long        */
...
VAL_SIGN *
#if PROTO
evalnum( const char * nump)
#else
evalnum( nump)
    char *  nump;                           /* Preprocessing number */
#endif
/*
 * Evaluate number for #if lexical analysis.  Note: evalnum recognizes
 ...
 */
{
    ...
    const char * const  out_of_range
            = "Constant \"%s\"%.0ld%s is out of range"; /* _E_ _W4_ */
    expr_t          value;
#if HAVE_ULONG
...
#if HAVE_ULONG
#if MODE != CON_FALSE
    if (uflag)                  /* If 'U' suffixed, uexpr_t i   */
        ev.sign = UNSIGNED;
    else
#if HAVE_LLONG && MODE >= STANDARD
    {
 ...
 */
{
    ...
    const char * const  w_out_of_range
        = "Wide character constant %s%.0ld%s is out of range";  /* _E_ _W4_ */
#endif
    const char * const  c_out_of_range
    = "Integer character constant %s%.0ld%s is out of range";   /* _E_ _W4_ */
#if HAVE_ULONG
...
#endif
    bits_save = bits;
    if ((cl = evalone( &cp, wide, (ucn8 = FALSE, &ucn8))) == -1L) {
        ev.sign = VAL_ERROR;
        return  & ev;
    }
...
#if HAVE_LLONG && MODE >= STANDARD
        if ((stdc_ver < 199901L && value > ULONGMAX)
                || ((value >> bits) < tmp))
#else
        if ((value >> bits) < tmp)
#endif
    ...
    if ((! skip && (warn_level & 1)) || (skip && (warn_level & 4))) {
#if MODE == STANDARD
        if (wide)
            cwarn(
"Multi-character wide character constant %s%.0ld%s isn't portable"  /* _W1_ _W4_    */
                    , token, 0L, skip ? non_eval : NULLST);
        else
            cwarn(
"Multi-character or multi-byte character constant %s%.0ld%s isn't portable" /* _W1_ _W4_    */
                    , token, 0L, skip ? non_eval : NULLST);
#else
        cwarn(
"Multi-character or multi-byte character constant %s%.0ld%s isn't portable" /* _W1_ _W4_    */
...
#define WCHARMAX    ((unsigned) ((UCHARMAX << CHARBIT) | UCHARMAX))

static expr_t
#if PROTO
evalone( char ** seq_pp, int wide, int * ucn8)
#else
evalone( seq_pp, wide, ucn8)
    char **     seq_pp;         /* Address of pointer to sequence   */
                    /* evalone() advances the pointer to sequence   */
    int     wide;                       /* Flag of wide-character   */
    int *   ucn8;                           /* Flag of UCN-32 bits  */
#endif
/*
 * Called from evalchar() above to get a single character or single wide
 * character (with or without \ escapes).
 * Returns the value of the character or -1L on error.
 */
{
#if MBCHAR
    static int      mb1seen = FALSE;
                        /* Preceded by the first byte of MBCHAR ?   */
#endif
#if MODE == STANDARD && OK_UCN
    const char * const  ucn_malval
        = "UCN cannot specify the value %.0s\"%08lx\""; /* _E_ _W4_ */
#endif
    const char * const  out_of_range
        = "%s%ld bits can't represent escape sequence '%s'";        /* _E_ _W4_ */
#if HAVE_ULONG
    uexpr_t         value;
#else
    expr_t          value;
...
#if MBCHAR
    if (! mb1seen && (type[ uc] & MB1)) {   /* 1st. byte of MBCHAR  */
        uc1 = **seq_pp & UCHARMAX;
        if (! (type[ uc1] & MB2))   /* Second byte of MBCHAR ?      */
            return  (expr_t) uc;            /* Already diagnosed    */
#if MODE == STANDARD
        if (wide) {
            (*seq_pp)++;
            return  (expr_t) ((uc << CHARBIT) | uc1);
        }
#endif
        mb1seen = TRUE;                     /* Remember the fact    */
        return  (expr_t) uc;
    } else if (mb1seen) {           /* Second byte of MBCHAR        */
        mb1seen = FALSE;
        return  (expr_t) uc;        /* '\\' isn't an escape sequence*/
    }
#endif  /* MBCHAR   */
...
undefined:
    ...
    **seq_pp = EOS;                         /* For diagnostic       */
    if ((! skip && (warn_level & 1)) || (skip && (warn_level & 4)))
        cwarn(
    "Undefined escape sequence%s %.0ld'%s'"         /* _W1_ _W4_    */
...
range_err:
    ...
    **seq_pp = EOS;                         /* For diagnostic       */
    ...
    if (wide) {
        if (! skip)
            cerror( out_of_range, NULLST, (long) CHARBIT * 2, seq);
        else if (warn_level & 4)
            cwarn( out_of_range, non_eval, (long) CHARBIT * 2, seq);
    } else {
        if (! skip)
 ...
 */
{
    const char * const  zero_div = "%sDivision by zero%.0ld%s";     /* _E_ _W4_ */
    const char * const  neg_long =
"Negative value \"%ld\" is converted to positive \"%lu\"%%s";       /* _W1_ _W4_*/
#if HAVE_LLONG && MODE >= STANDARD
    const char * const  neg_llong =
            "Negative value \"%" LL_FORM "d\" is converted to positive \"%"
                    LL_FORM "u\"%%s";       /* _W1_ _W4_*/
#endif
...
#if DEBUG_EVAL
    if (debug & EXPRESSION) {
        printf( "%s op %s", (is_binary( op)) ? "binary" : "izary"
                , opname[ op]);
        dumpval( ", v1 = ", valp);
        if (is_binary( op))
            dumpval( ", v2 = ", valp + 1);
...
#if HAVE_ULONG                      /* Usual arithmetic conversion  */
...
#if HAVE_LLONG && MODE >= STANDARD
                || (stdc_ver < 199901L && v2 >= sizeof (long) * CHARBIT)
#endif
            ) && ((!skip && (warn_level & 1)) || (skip && (warn_level & 4))))
        cwarn( "Illegal shift count %.0s\"%ld\"%s"          /* _W1_ _W4_    */
...
#if HAVE_ULONG
    if (sign1 == SIGNED)
        v1 = evalsval( & valp, v1, v2, op);
    else
        v1 = evaluval( & valp, (uexpr_t) v1, (uexpr_t) v2, op);
#else
    v1 = evalsval( & valp, v1, v2, op);
#endif
...
static expr_t
#if PROTO
evalsval( VAL_SIGN ** valpp, expr_t v1, expr_t v2, int op)
#else
evalsval( valpp, v1, v2, op)
    VAL_SIGN ** valpp;
    expr_t      v1, v2;
    int         op;
#endif
 ...
 */
{
    ...
    const char * const   illeg_op
        = "Bug: Illegal operator \"%s\" in evalsval()";     /* _F_  */
#endif
    const char * const  not_portable
        = "\"%s\" of negative number isn't portable%.0ld%s";    /* _W1_ _W4_    */
    ...
    case OP_NEG:
        if ((v1 && v1 == -v1)
#if HAVE_LLONG && MODE >= STANDARD
                || (stdc_ver < 199901L && (long) v1 == (long) -v1)
#endif
            )
    ...
    case OP_DIV:
    case OP_MOD:
        /* Division by 0 has been already diagnosed by evaleval().  */
...
static expr_t
#if PROTO
evaluval( VAL_SIGN ** valpp, uexpr_t v1u, uexpr_t v2u, int op)
#else
evaluval( valpp, v1u, v2u, op)
    VAL_SIGN **     valpp;
    uexpr_t     v1u, v2u;
    int         op;
#endif
 ...
 */
{
#if DEBUG_EVAL
    const char * const   illeg_op
        = "Bug: Illegal operator \"%s\" in evaluval()";     /* _F_  */
#endif
    const char *    op_name = opname[ op];
    uexpr_t     v1;
    ...
    case OP_COL:    v1 = (--*valpp)->val ? v1u : v2u; break;
#if DEBUG_EVAL
    default:
        cfatal( illeg_op, op_name, 0L, NULLST);
#endif
...
{
    const char * const  out_of_range
        = "Result of \"%s\" is out of range%.0ld%s";    /* _E_ _W1_ _W4_    */
...
static void
#if PROTO
dumpval( const char * msg, const VAL_SIGN * valp)
#else
dumpval( msg, valp)
    char *      msg;
    VAL_SIGN *  valp;
 ...
 */
{
    ...
    const char * const  format_ll
                = "%s(%ssigned long long) 0x%08" LL_FORM "x";
#endif
...
#if HAVE_LLONG && MODE >= STANDARD
    if (stdc_ver >= 199901L) {
        printf( format_ll, msg, sign ? "" : "iz", valp->val);
    } else {
        printf( format, msg, sign ? "" : "iz", (long) valp->val);
    }
#else
    printf( format, msg, sign ? "" : "iz", valp->val);
#endif
}

static void
#if PROTO
dumpstack( const OPTAB * opstack, const OPTAB * opp, const VAL_SIGN * value
        , const VAL_SIGN * valp)
#else
dumpstack( opstack, opp, value, valp)
 ...
 */
{
    ...
    while (opstack < opp) {
        printf( " [%2d] %2d %04o    %d %s\n", (int)(opp - opstack)
                , opp->op, opp->prec, opp->skip, opname[ opp->op]);
        opp--;
    }

    while (value <= --valp) {
        printf( "value[%d].val = ", (int)(valp - value));
        dumpval( "", valp);
        fputc( '\n', stdout);
    }
}
