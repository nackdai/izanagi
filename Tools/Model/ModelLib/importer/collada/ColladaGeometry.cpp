	...
	std::vector<SSkin>& tvSkinList)
{
	...
	for (size_t i = 0; i < pWeightsElement->getInput_array().getCount(); i++) {
		...
		if (izanagi::izanagi_tk::CStringUtil::CmpStr(strSemantic, "WEIGHT")) {
...
namespace {
		...
		domString strSrcId)
	{
		...
		for (size_t i = 0; i < num; i++) {
			...
			if (izanagi::izanagi_tk::CStringUtil::CmpStr(pSrc->getID(), strSrcId)) {
				return pSrc;
			}
		}
		...
		izanagi::SVector& sVec)
	{
		...
		E_VTX_ACCESS_TYPE ret = E_VTX_ACCESS_TYPE_NUM;
		{
			...
			for (IZ_UINT i = 0; i < E_VTX_ACCESS_TYPE_NUM; i++) {
				if (izanagi::izanagi_tk::CStringUtil::CmpStr(TypeTbl[i], type)) {
	...
	_T pPrim)
{
	...
	for (size_t i = 0; i < nInputNum; i++) {
		...
		if (izanagi::izanagi_tk::CStringUtil::CmpStr(strSemantic, "VERTEX")) {
			// <vertices> 経由
	...
	IZ_UINT nVtxIdx)
{
	...
	for (size_t i = 0; i < nInputNum; i++) {
		...
		else if (izanagi::izanagi_tk::CStringUtil::CmpStr(strSemantic, "POSITION")) {
			nVtxOffset = (IZ_UINT)pInput->getOffset();
			break;
		}
	}
