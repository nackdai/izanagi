...
struct CColladaJoint::SFuncIsSameNameNode {
	bool operator()(domNodeRef node)
	{
		...
		IZ_BOOL ret = izanagi::izanagi_tk::CStringUtil::CmpStr(name, strName);
		...
		if (pNode->getInstance_controller_array().getCount() > 0) {
			...
			if (pSkin != IZ_NULL) {
				...
				for (size_t nInputPos = 0; nInputPos < nInputNum; nInputPos++) {
					...
					if (izanagi::izanagi_tk::CStringUtil::CmpStr(strSemantic, "JOINT")) {
					...
					else if (izanagi::izanagi_tk::CStringUtil::CmpStr(strSemantic, "INV_BIND_MATRIX")) {
...
namespace {
	struct SFuncIsSameNameJoint {
		...
		{
			IZ_BOOL ret = izanagi::izanagi_tk::CStringUtil::CmpStr(
...
E_MDL_JOINT_TRANSFORM CColladaJoint::GetTransformType(IZ_PCSTR pszName)
{
	...
	for (IZ_UINT i = 0; i < E_MDL_JOINT_TRANSFORM_NUM; i++) {
		if (izanagi::izanagi_tk::CStringUtil::CmpStr(tblType[i].str, pszName)) {
