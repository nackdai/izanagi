...
namespace {
	...
	// Dimensions
	BOOL _SetValueDimensions(izanagi::S_PES_TEXTURE_ANN& sAnn, CGannotation ann)
	{
		INT num;
	...
	// ViewportRatio
	BOOL _SetValueViewportRatio(izanagi::S_PES_TEXTURE_ANN& sAnn, CGannotation ann)
	{
		INT num;
		const float* p = cgGetFloatAnnotationValues(ann, &num);
		if (num == 2) {
	...
	// IsRenderTarget
	BOOL _SetValueIsRenderTarget(izanagi::S_PES_TEXTURE_ANN& sAnn, CGannotation ann)
	{
		int num;
	...
	// IsDynamic
	BOOL _SetValueIsDynamic(izanagi::S_PES_TEXTURE_ANN& sAnn, CGannotation ann)
	{
		int num;
		const CGbool* p = cgGetBoolAnnotationValues(ann, &num);
		if (p != NULL) {
	...
	// RscType
	BOOL _SetValueRscType(izanagi::S_PES_TEXTURE_ANN& sAnn, CGannotation ann)
	{
		static struct {
			IZ_PCSTR strFmt;
